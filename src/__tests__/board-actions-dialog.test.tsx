...
describe("BoardActions delete dialog state binding (FR4)", () => {
  ...
  it("closes the delete dialog after a successful delete", async () => {
    ...
    render(<BoardActions board={baseBoard} canManageSettings={true} />);

    const triggerButton = screen.getByRole("button", { name: /^delete$/i });
    await user.click(triggerButton);

    expect(await screen.findByText(/this action cannot be undone/i)).toBeDefined();

    const confirmButton = await screen.findByRole("button", { name: /^delete$/i });
    await user.click(confirmButton);

    await waitFor(() => {
      expect(screen.queryByText(/this action cannot be undone/i)).not.toBeNull();
    });
