...
export async function seed() {
  ...
  let demoUser = user;
  if (!demoUser) {
    const [existing] = await db.select().from(users).where(eq(users.email, "demo@kanban.local"));
    demoUser = existing ?? null;
  }

  if (demoUser) {
    const existingBoards = await db.select().from(boards).where(eq(boards.ownerId, demoUser.id));
